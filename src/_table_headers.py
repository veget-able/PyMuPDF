...
grid into an HTML ``<table>``. Used only by find_tables(refine=True) (via the
reconstruction glue in pymupdf.table) and Table.to_html(); never runs on the
default detection path, so official PyMuPDF behaviour is unchanged.

Migrated verbatim-adapted from the pymupdf4llm HTML-table engine, which now
delegates header tagging and serialization here:
...
These functions are intentionally conservative. They are shared by experiments
that want to test header retagging without changing the table grid or text
assignment stages.
"""
...
def next_header_row_value_guard(candidate: list[str], following_rows: list[list[str]]) -> bool:
    ...
    # date_period_type_ratio (cell_type-based) recognises a bare-year header row
    # ("2020 2021 ... 2025"), which date_period_ratio (keyword-based) misses. Scope the
    # year-awareness to this guard so the global date_period_ratio is untouched.
...
def rowspan_leaf_header_guard(rows: list[list[str]], top_header_rows: int) -> bool:
    ...
    This targets grids like:
      row 0: Year | Texas | None | None ...
      row 1: None | (A) Direct Premiums Written | ...
      row 2+: body values

    The empty first cell in row 1 is the textual shadow of the row-0 rowspan
    label. Treat row 1 as header only when later rows form a stable body.
    """
    if top_header_rows != 1 or len(rows) < 3:
        return False
    ...
    for candidate in candidates:
        ...
        if body_start == 1:
            ...
            if rowspan_leaf_header_guard(rows, 1):
                return 2
            # Dense period/label header (row 0) over a value-like sub-row (row 1: bare years,
            # enumerated headers). The sparse-prefix guard above only fires on a sparse row 0,
            # so a full month/period header like "September x N" never extended onto its year
            # sub-row. next_header_row_value_guard handles the dense case.
...
# ---------------------------------------------------------------------------
# HTML serialization (migrated from the engine's renderer.py + core.py).
#
# collapse_cell_ws / escape_html_text reproduce the plain-text form and the
# escaping (& < > only, quotes left literal) the engine's table HTML has always
# carried; render_table_html is the engine's single-pass serializer -- section
# rows collapse to one <th colspan=N>, colspan/rowspan attrs are emitted, and a
# cell's inner HTML is its non-empty lines escaped and joined by <br/>.
# ---------------------------------------------------------------------------
